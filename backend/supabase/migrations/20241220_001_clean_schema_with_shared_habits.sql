-- Migration: 20241220_001_clean_schema_with_shared_habits.sql
-- Clean schema with Supabase Auth + shared habits functionality
...
-- ========================================
-- HABIT TRACKING (with sharing)
-- ========================================

-- Habits table (with sharing capabilities)
CREATE TABLE IF NOT EXISTS habits (
    ...
    shared_with UUID[] DEFAULT '{}', -- Array of user IDs who can see this habit
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Habit sharing requests (for requesting access to shared habits)
CREATE TABLE IF NOT EXISTS habit_sharing_requests (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    habit_id UUID REFERENCES habits(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_habits_shared_with ON habits USING GIN(shared_with);
CREATE INDEX IF NOT EXISTS idx_habit_completions_user_date ON habit_completions(user_id, date);
CREATE INDEX IF NOT EXISTS idx_habit_sharing_requests_requester ON habit_sharing_requests(requester_id);
CREATE INDEX IF NOT EXISTS idx_habit_sharing_requests_owner ON habit_sharing_requests(owner_id);
...
CREATE INDEX IF NOT EXISTS idx_habit_sharing_requests_status ON habit_sharing_requests(status);
...
ALTER TABLE habit_sharing_requests ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Service role can access habit_sharing_requests" ON habit_sharing_requests
    FOR ALL USING (auth.role() = 'service_role');
...
-- Habits: Users can see their own habits + shared habits
CREATE POLICY "Users can access own habits" ON habits
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users can access shared habits" ON habits
    FOR SELECT USING (auth.uid() = ANY(shared_with));

-- Habit completions: Users can only see their own completions
CREATE POLICY "Users can access own habit_completions" ON habit_completions
    FOR ALL USING (auth.uid() = user_id);

-- Habit sharing requests: Users can see requests they made or received
CREATE POLICY "Users can access own habit_sharing_requests" ON habit_sharing_requests
    FOR ALL USING (auth.uid() = requester_id OR auth.uid() = owner_id);
...
-- ========================================
-- HELPER FUNCTIONS FOR SHARED HABITS
-- ========================================

-- Function to share a habit with another user
CREATE OR REPLACE FUNCTION share_habit_with_user(
    habit_id_param UUID,
    target_user_id UUID
)
RETURNS BOOLEAN AS $$
...
BEGIN
    -- Get the habit owner
    SELECT user_id INTO habit_owner 
    FROM habits 
    WHERE id = habit_id_param;
    
    -- Check if the current user owns the habit
    IF habit_owner != auth.uid() THEN
        RAISE EXCEPTION 'You can only share habits you own';
    END IF;
...
-- Function to request access to a shared habit
CREATE OR REPLACE FUNCTION request_habit_access(
    habit_id_param UUID
)
RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
    ...
    -- Check if habit is shared
    IF NOT EXISTS (
        SELECT 1 FROM habits 
        WHERE id = habit_id_param AND is_shared = true
    ) THEN
        RAISE EXCEPTION 'This habit is not shared';
    END IF;
    
    -- Check if user already has access
    IF EXISTS (
        SELECT 1 FROM habits 
        WHERE id = habit_id_param AND auth.uid() = ANY(shared_with)
    ) THEN
        RAISE EXCEPTION 'You already have access to this habit';
    END IF;
    
    -- Create sharing request
    INSERT INTO habit_sharing_requests (habit_id, requester_id, owner_id)
    VALUES (habit_id_param, auth.uid(), habit_owner)
    ON CONFLICT (habit_id, requester_id) DO NOTHING;
    
    RETURN TRUE;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to approve habit sharing request
CREATE OR REPLACE FUNCTION approve_habit_sharing_request(
    request_id_param UUID
)
RETURNS BOOLEAN AS $$
DECLARE
    request_record habit_sharing_requests%ROWTYPE;
BEGIN
    -- Get the request
    SELECT * INTO request_record 
    FROM habit_sharing_requests 
    WHERE id = request_id_param;
    
    -- Check if current user is the owner
    IF request_record.owner_id != auth.uid() THEN
        RAISE EXCEPTION 'You can only approve requests for your own habits';
    END IF;
    
    -- Update request status
    UPDATE habit_sharing_requests 
    SET status = 'approved', responded_at = NOW()
    WHERE id = request_id_param;
    
    -- Add user to shared_with array
    UPDATE habits 
    SET shared_with = array_append(shared_with, request_record.requester_id)
    WHERE id = request_record.habit_id;
...
COMMENT ON TABLE habits IS 'User-defined habits to track (with sharing capabilities)';
COMMENT ON TABLE habit_completions IS 'Daily habit completion records';
COMMENT ON TABLE habit_sharing_requests IS 'Requests to access shared habits';
